...
func (f *FAQs) Create(ctx context.Context, businessID int64, question, answer, category, language string) (int64, error) {
	...
	const q = `
		INSERT INTO faqs (business_id, question, answer, embedding, category, language)
		VALUES ($1, $2, $3, $4::vector, $5, $6)
		RETURNING id`

	var id int64
	if err := f.pool.QueryRow(ctx, q,
		businessID, question, answer, encodeVector(emb), nullIfEmpty(category), normLang(language),
	).Scan(&id); err != nil {
...
func (f *FAQs) Search(ctx context.Context, businessID int64, query, language string, limit int) ([]core.Match, error) {
	...
	const q = `
		...
		FROM faqs
		WHERE business_id = $1 AND language = $4 AND embedding IS NOT NULL
		ORDER BY embedding <=> $2::vector
		LIMIT $3`

	rows, err := f.pool.Query(ctx, q, businessID, encodeVector(emb), limit, normLang(language))
	if err != nil {
...
// normLang defaults a blank language to English and lowercases the code.
...
func (f *FAQs) List(ctx context.Context, businessID int64, language string) ([]FAQ, error) {
	const q = `
		SELECT id, question, answer, coalesce(category, ''), language
		FROM faqs
		WHERE business_id = $1 AND language = $2
		ORDER BY updated_at DESC`

	rows, err := f.pool.Query(ctx, q, businessID, normLang(language))
	if err != nil {
	...
	for rows.Next() {
		var it FAQ
		if err := rows.Scan(&it.ID, &it.Question, &it.Answer, &it.Category, &it.Language); err != nil {
...
// Languages returns the distinct languages a business has authored FAQs in,
// so the editor and widget can show only languages that actually have content.
func (f *FAQs) Languages(ctx context.Context, businessID int64) ([]string, error) {
	const q = `SELECT DISTINCT language FROM faqs WHERE business_id = $1 ORDER BY language`
...
func (f *FAQs) Categories(ctx context.Context, businessID int64, language string) ([]string, error) {
	const q = `
		SELECT category
		FROM faqs
		WHERE business_id = $1 AND language = $2 AND category IS NOT NULL AND category <> ''
		GROUP BY category
		ORDER BY min(id)`
...
func (f *FAQs) ListByCategory(ctx context.Context, businessID int64, category, language string) ([]FAQ, error) {
	const q = `
		SELECT id, question, answer, coalesce(category, ''), language
		FROM faqs
		WHERE business_id = $1 AND category = $2 AND language = $3
		ORDER BY id`

	rows, err := f.pool.Query(ctx, q, businessID, category, normLang(language))
	if err != nil {
...
func (f *FAQs) GetByID(ctx context.Context, businessID, id int64) (FAQ, error) {
	...
	var it FAQ
	if err := f.pool.QueryRow(ctx, q, businessID, id).Scan(&it.ID, &it.Question, &it.Answer, &it.Category, &it.Language); err != nil {
...
func (f *FAQs) Update(ctx context.Context, businessID, id int64, question, answer, category, language string) error {
	emb, err := f.embedder.Embed(ctx, question)
	if err != nil {
	...
	const q = `
		UPDATE faqs
		SET question = $3, answer = $4, embedding = $5::vector, category = $6, language = $7, updated_at = now()
		WHERE business_id = $1 AND id = $2`
	if _, err := f.pool.Exec(ctx, q, businessID, id, question, answer, encodeVector(emb), nullIfEmpty(category), normLang(language)); err != nil {
