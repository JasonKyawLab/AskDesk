...
func Load() (*Config, error) {
	...
	// Contact-gate mode: off | always | handoff. ASKDESK_REQUIRE_CONTACT=true is
	// a legacy alias for "always".
	switch cc := strings.ToLower(getEnv("ASKDESK_CONTACT_CAPTURE", "")); cc {
	case "always", "handoff", "off":
		cfg.ContactCapture = cc
	case "":
		if getEnvBool("ASKDESK_REQUIRE_CONTACT", false) {
			cfg.ContactCapture = "always"
		} else {
			cfg.ContactCapture = "off"
		}
	default:
