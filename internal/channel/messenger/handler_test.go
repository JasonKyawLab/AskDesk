...
func TestHandler_VerifyHandshake(t *testing.T) {
	h := NewHandler(&fakeSubmitter{}, nil, nil, nil, nil, 1, "en", "sekret", "verify-me", discardLogger())
...
func TestHandler_NormalizesAndSubmits(t *testing.T) {
	sub := &fakeSubmitter{}
	h := NewHandler(sub, nil, nil, nil, nil, 1, "en", "sekret", "v", discardLogger())
...
func TestHandler_IgnoresEchoAndNonText(t *testing.T) {
	sub := &fakeSubmitter{}
	h := NewHandler(sub, nil, nil, nil, nil, 1, "en", "", "v", discardLogger()) // empty secret: signature check disabled
